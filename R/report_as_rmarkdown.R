...
print("plot class for gg cscore")
  ...
  for (contr in l_contrast) {
    ...
    for (i in seq_along(contr)){
        ...
        for (m in seq_along(plot_list)){
          ...
          if (isa(plot_gg, c("gg","ggplot"))){
            ...
            if (isa(plot_gggg, c("gg","ggplot"))){
              ...
              jplotly_c <- class(plot_json)
              print(jplotly_c)
              filename_json <- paste0(plot_dir, "/plotv_", k, "_", i,"_", m, ".json")
              write(plot_json, file = filename_json)
              filename_html <- paste0(plot_dir, "/plotv_", k, "_", i,"_", m, ".html")
              htmlwidgets::saveWidget(plot_plotly, filename_html)
            }
          }
        }
      ...
      if (isa(plot_gg, c("gg","ggplot"))){
        ...
        jplotly_c <- class(plot_json)
        print(jplotly_c)
        filename_json <- paste0(plot_dir, "/plothf_", k, "_", i,"_", m, ".json")
        write(plot_json, file = filename_json)
        filename_html <- paste0(plot_dir, "/plothf_", k, "_", i,"_", m, ".html")
        htmlwidgets::saveWidget(plot_plotly, filename_html)
      }
    }
  }
  ...
  for (plot_gg in dd_plots) {
    ...
    if (isa(plot_gg, c("gg","ggplot"))){
      ...
      jplotly_c <- class(plot_json)
      print(jplotly_c)
      filename_json <- paste0(plot_dir, "/plotdd_", k, ".json")
      write(plot_json, file = filename_json)
      filename_html <- paste0(plot_dir, "/plotdd_", k, ".html")
      htmlwidgets::saveWidget(plot_plotly, filename_html)
    }
  }
